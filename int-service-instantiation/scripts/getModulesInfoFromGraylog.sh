...
LOGMESSAGE=$(curl -X GET "http://admin:s0n%40t%40@10.31.11.37:12900/search/universal/keyword?query=container_name%3Aservicelifecyclemanagement%20AND%20message%3A*Deployment*%20AND%20message%3A*reply*%20AND%20message%3A*received*&keyword=last%205%20minutes&fields=container_name%2Cmessage")
...
LOGMESSAGE=$(curl -X GET "http://admin:s0n%40t%40@10.31.11.37:12900/search/universal/keyword?query=container_name%3Aservicelifecyclemanagement%20AND%20message%3A*inform*%20AND%20message%3A*gk*%20AND%20message%3A*result*&keyword=last%205%20minutes&fields=container_name%2Cmessage")
echo $LOGMESSAGE >> $REP_DIR/intermediate_Info.html
# info: uppercase(LOGMESSAGE) and delete duplicated spaces
LOGMESSAGE=$(echo "${LOGMESSAGE^^}" | tr -s " ")
