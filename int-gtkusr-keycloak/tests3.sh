...
-H "Content-Type: application/x-www-form-urlencoded" \
-X POST 'http://sp.int3.sonata-nfv.eu:5600/api/v1/authorize?path=/services&method=POST') 2>/dev/null
echo $resp
...
resp=$(curl -qSfsw '\n%{http_code}' -H "Authorization: Bearer $token" \
-H "Content-Type: application/x-www-form-urlencoded" \
-X POST 'http://sp.int3.sonata-nfv.eu:5600/api/v1/authorize?path=/requests&method=GET') 2>/dev/null
echo $resp
