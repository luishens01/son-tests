...
docker run -i -e DATABASE_HOST=sp.int3.sonata-nfv.eu -e RACK_ENV=integration -e DATABASE_HOST=sp.int3.sonata-nfv.eu -e DATABASE_PORT=5432 -e POSTGRES_PASSWORD=sonata -e POSTGRES_USER=sonatatest --rm=true registry.sonata-nfv.eu:5000/son-gtksrv bundle exec rake db:migrate
echo 
...
echo gtkapi
docker pull registry.sonata-nfv.eu:5000/son-gtkapi
docker run --name son-gtkapi -d -p 32001:5000 --add-host sp.int.sonata-nfv.eu:10.31.11.33 --link son-gtkpkg --link son-gtksrv -e RACK_ENV=integration -e PACKAGE_MANAGEMENT_URL=http://sp.int3.sonata-nfv.eu:5100 -e SERVICE_MANAGEMENT_URL=http://sp.int3.sonata-nfv.eu:5300 registry.sonata-nfv.eu:5000/son-gtkapi 
