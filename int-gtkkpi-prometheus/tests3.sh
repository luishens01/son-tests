...
# KPIs list size
counter_value=$(echo $resp | python -c 'import json,sys;obj=json.load(sys.stdin);print obj')
echo "There are $counter_value registered KPIs in the system"

if [[ $counter_value -lt 2 ]];  then
	echo "Error: KPIs list does not contain the test elements"
	exit -1
fi
